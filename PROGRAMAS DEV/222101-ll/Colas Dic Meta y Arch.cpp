...
void guarda_cola_txt(Diccionario dic, char nom[], char nomcol[], Cola c);
void abrir_cola_txt(Diccionario *dic, char nom[], char nomcol[], Cola *c);
void guarda_cola_bin(Diccionario dic, char nom[], char nomcol[], Cola c);
void abrir_cola_bin(Diccionario *dic, char nom[], char nomcol[], Cola *c);
void guarda_archivo_cola(Diccionario dic, char nom[], char nomcol[], Cola c);
void abrir_archivo_cola(Diccionario *dic, char nom[], char nomcol[], Cola *c);
     ...
     do{
		 ...
		 switch(opcion)
		 {
            ...
            case 6: system("cls");
                    ...
                    pide_nombre(nom_arch_cola);
                    guarda_archivo_cola(m_dic, nom_arch, nom_arch_cola, c);
                    break;
          
            case 7: system("cls");
                    printf("\nNombre Archivo Datos\n");
                    pide_nombre(nom_arch_cola);
                    abrir_archivo_cola(&m_dic, nom_arch, nom_arch_cola, &c);
                    imprimeCola(m_dic, c);
                    system("pause");
                    break;  
			}
...
void guarda_archivo_cola(Diccionario dic, char nom[], char nomcol[], Cola c)
{
     ...
     if(tipo=='T' || tipo=='t')
        guarda_cola_txt(dic, nom, nomcol, c);
     else
        guarda_cola_bin(dic, nom, nomcol, c);
}
...
void abrir_archivo_cola(Diccionario *dic, char nom[], char nomcol[], Cola *c)
{
     ...
     if(tipo=='T' || tipo=='t')
        abrir_cola_txt(dic, nom, nomcol, c);
     else
        abrir_cola_bin(dic, nom, nomcol, c);
}
...
void guarda_cola_txt(Diccionario dic, char nom[], char nomcol[], Cola c)
{
    ...
    f=fopen(nomcol,"w");
    fprintf(f, "%s ", nom);
    fprintf(f, "%d ", c.tamDatos);
    fprintf(f, "%d ", c.tam);
    
    dir = c.cola;
    for(i=0; i<c.tamDatos; i++)
    {
...
void abrir_cola_txt(Diccionario *dic, char nom[], char nomcol[], Cola *c)
{
    ...
    rewind(f);
    fscanf(f, "%s", nom);
    fscanf(f, "%d", &c->tamDatos);
    fscanf(f, "%d", &c->tam);
    
    abrir_texto(dic, nom);
    c->cola = malloc(c->tam * c->tamDatos);
...
void guarda_cola_bin(Diccionario dic, char nom[], char nomcol[], Cola c)
{
    ...
    f=fopen(nomcol,"wb");
    fwrite(nom, 50, 1, f);
    fwrite(&c.tamDatos, sizeof(int), 1, f);
    fwrite(&c.tam, sizeof(int), 1, f);
    
    fwrite(c.cola, c.tam, c.tamDatos, f);    
...
void abrir_cola_bin(Diccionario *dic, char nom[], char nomcol[], Cola *c)
{
    ...
    fread(nom, 50, 1, f);
    fread(&c->tamDatos, sizeof(int), 1, f);
    fread(&c->tam, sizeof(int), 1, f);
    
    abrir_bin(dic, nom);
    
    c->cola = malloc(c->tam * c->tamDatos);
    
    fread(c->cola, c->tam, c->tamDatos, f); 
